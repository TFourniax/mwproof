...
from .event_ledger import AUTHORITATIVE_SOURCE_CLASSES, MilestoneObservation, resolved_forecast_pairs
...
def research_priorities(items: Iterable[MilestoneObservation]) -> dict[str, Any]:
    ...
    Control counts deliberately reuse the conservative interval-aware readiness
    labels. This keeps acquisition targets consistent with publication gates.
    """
    rows = list(items)
    ...
    authoritative = sum(1 for row in rows if row.source_class in AUTHORITATIVE_SOURCE_CLASSES)
    ...
    controls_early = int(labels["certainly_early_or_on_time"])
    controls_delayed = int(labels["certainly_materially_delayed"])
    controls_ambiguous = int(labels["interval_ambiguous"])
    ...
    if gaps["authoritative_events_to_50pct"]:
        recommendations.append({"priority": 1, "objective": "increase_primary_provenance", "target_additions": gaps["authoritative_events_to_50pct"], "preferred_sources": ["regulators", "grid operators", "company filings", "developer releases", "customer confirmations", "OEM releases"], "why": "Primary evidence improves defensibility and reduces dependence on trade press."})
    ...
    recommendations.append({"priority": 2, "objective": "balance_controls", "target_additions": {"on_time_or_early": gaps["early_or_on_time_controls"], "materially_delayed": gaps["materially_delayed_controls"]}, "ambiguous_resolved_controls": controls_ambiguous, "why": "Only interval-certain controls count toward readiness; wide intervals that cross the 90-day threshold stay ambiguous."})
    ...
    return {
        ...
        "current": {"observations": len(rows), "projects": total_projects, "resolved_operations": len(ops), "unique_sources": quality["scope"]["unique_sources"], "authoritative_events": authoritative, "authoritative_ratio": quality["provenance"]["authoritative_or_first_party_ratio"], "certainly_early_or_on_time_controls": controls_early, "certainly_materially_delayed_controls": controls_delayed, "interval_ambiguous_controls": controls_ambiguous},
        "gaps": gaps,
