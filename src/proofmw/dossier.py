...
def project_dossier(items: Iterable[MilestoneObservation], project_id: str, *, as_of: str | None = None) -> dict[str, Any]:
    ...
    for milestone_id, group in sorted(milestone_rows.items()):
        ...
        actuals = [x for x in group if x.status == "actual"]
        ...
        milestones.append({
            ...
            "actual": ({
                "observed_on": latest_actual.observed_on,
                "actual_date": latest_actual.actual_date,
                "actual_start": latest_bounds[0].isoformat() if latest_bounds else latest_actual.actual_start,
                "actual_end": latest_bounds[1].isoformat() if latest_bounds else latest_actual.actual_end,
                "precision": latest_actual.precision,
                "capacity_mw": latest_actual.capacity_mw,
                "source_url": latest_actual.source_url,
                "source_class": latest_actual.source_class,
                "temporally_scorable": latest_bounds is not None,
    ...
    return {
        ...
        "evidence_summary": {"observations": len(rows), "unique_sources": len({x.source_url for x in rows}), "authoritative_or_first_party_observations": sum(1 for x in rows if x.source_class in AUTHORITATIVE_SOURCE_CLASSES), "source_conflicts": len(conflicts), "missing_physical_milestone_types": missing_physical_types, "stale_unresolved_forecasts": stale_forecasts, "confirmed_actual_without_scorable_time": sum(1 for x in rows if x.status == "actual" and actual_window(x) is None)},
        ...
        "guardrail": "This dossier is an evidence chronology, not a credit opinion. Missing evidence and stale forecasts remain explicit rather than being imputed as facts. A confirmed actual state without a defensible physical date is visible but excluded from temporal scoring.",
    }
